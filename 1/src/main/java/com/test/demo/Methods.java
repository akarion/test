...
//				driver.findElement(By.id("arrivalFieldInput")).sendKeys("\n");
				Thread.sleep(1000);
				driver.findElement(By.id("freeFormatCityEntry")).click();
				driver.findElement(By.id("freeFormatCityEntry")).sendKeys("\n");
...
//		driver.findElement(By.name("search")).click();
	...
	public void verifyStarRating(int ratingIndex, WebDriver driver) throws InterruptedException{
		...
		*/
		...
		System.out.println("Current selected Star Rating index is: "+ratingIndex);
		...
		while(otherIndexes<=5){
			...
			List<WebElement> elems =  driver.findElements(By.xpath("//img[@alt='"+otherIndexes+" Star']"));
			
//			System.out.println("Collected list size: "+elems.size());   //used for debug
			
			if(elems.size()>0){
				System.out.println("Contents of the list: "+elems.toString());
				
			}
				
			if (elems.isEmpty()==true){
				status = "OK: filter works";
			}
			...
			for (WebElement elem : elems){
				...
				if (elem.isDisplayed()==false | otherIndexes== ratingIndex+1){
					
					status = "OK: filter works after second check";
					
					
				} else{
					
					status = "Not ok- element is displayed and has wrong star rating";
...
//					continue;
			...
			System.out.println("Status for index "+otherIndexes+" : "+status);
	...
	public String[] generateWord(){
		...
		String russian[] = {"Балчуг","Путин","Корова","Яблоко","Калашников","Водка","Селедка","Лада","Россия","Нефть","Мёд","Орех"};
		String arabic[] = {"بوتين","فودكا","مكة المكرمة","المدينة المنورة","صلاح الدين","القاهرة","الإسكندرية"};
		String japanese[] = {"ロシア","ケーキ","ピカチュウ", "サムライ","神道", "東京", "ドミトリー", "プーチン" };
		
	int ruNameIndex = new Random().nextInt(russian.length);
	String randomNamePartOne = russian[ruNameIndex];
	
	int aNameIndex = new Random().nextInt(arabic.length);
	String randomNamePartTwo = arabic[aNameIndex];
	
	int jNameIndex = new Random().nextInt(japanese.length);
	String randomNamePartThree = japanese[jNameIndex];
	
	String arrayName[] = new String[3]; 
	arrayName[0]= randomNamePartOne;
	arrayName[1]= randomNamePartTwo;
	arrayName[2]= randomNamePartThree;
